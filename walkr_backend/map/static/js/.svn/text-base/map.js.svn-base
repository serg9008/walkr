						...
						if(num-1 == index) 
							...
							$("#imageSlider img").each(function() {
								...
								var marker = new google.maps.Marker({
									...
									icon: '/static/images/camera_icon.png'
								    });
...
function addImage(item, container) {
    ...
    var el2 = $(container).find('img');
    if(el.length || (el2.length && el2.length > 0))
	return;
    var nimgHeight = 75;
    ...
    if($("#stopName").val()=="")
	$("#stopName").val(currPhoto[$(item).attr('id')].name);
    $("#buttons").show();
    if($("#photoContainer").find('td').length == $("#photoContainer").find('img').length) {
	var imgRow = $('<tr> <td align="center" class="drag-bg"></td><td align="center" class="drag-bg"></td></tr>');
	$("#photoTable").append(imgRow);
	initDropArea();
    } 
...
$(document).ready(function() {
	...
	$("#saveButton").button();
		...
		$.post('/walkr/savestop', 
		       { urls: urlsArray, lat: photo.lat, lon: photo.lng, 
			       name: (($("#stopName").val()=="")? photo.name : $("#stopName").val()), desc: $("#descStop").val(),
